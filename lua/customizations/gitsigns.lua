-- gitsigns with an IntelliJ-style gutter.
--
-- A thick bar marks added and changed lines, a triangle marks where lines were deleted;
-- colours come from the colorscheme (gruvbox: green, aqua, red). The signs are part of the
-- `setup` call, so this file owns the whole gitsigns configuration, including the hunk
-- keymaps kickstart recommends (<leader>h..., ]c / [c, `ih` text object). The `<leader>h`
-- which-key group stays in init.lua because it is stock kickstart.
